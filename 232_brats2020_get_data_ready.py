...
sample_dirs = sorted([d for d in os.listdir(TRAIN_DATASET_PATH) if os.path.isdir(os.path.join(TRAIN_DATASET_PATH, d))])
sample_folder = sample_dirs[0] if sample_dirs else 'BraTS2021_00000'
sample_path = os.path.join(TRAIN_DATASET_PATH, sample_folder, sample_folder)

test_image_flair=nib.load(sample_path + '_flair.nii.gz').get_fdata()
print(test_image_flair.max())
#Scalers are applied to 1D so let us reshape and then reshape back to original shape. 
test_image_flair=scaler.fit_transform(test_image_flair.reshape(-1, test_image_flair.shape[-1])).reshape(test_image_flair.shape)


test_image_t1=nib.load(sample_path + '_t1.nii.gz').get_fdata()
test_image_t1=scaler.fit_transform(test_image_t1.reshape(-1, test_image_t1.shape[-1])).reshape(test_image_t1.shape)

test_image_t1ce=nib.load(sample_path + '_t1ce.nii.gz').get_fdata()
test_image_t1ce=scaler.fit_transform(test_image_t1ce.reshape(-1, test_image_t1ce.shape[-1])).reshape(test_image_t1ce.shape)

test_image_t2=nib.load(sample_path + '_t2.nii.gz').get_fdata()
test_image_t2=scaler.fit_transform(test_image_t2.reshape(-1, test_image_t2.shape[-1])).reshape(test_image_t2.shape)

test_mask=nib.load(sample_path + '_seg.nii.gz').get_fdata()
...
t2_list = sorted(glob.glob('Data/*/*t2.nii.gz'))
t1ce_list = sorted(glob.glob('Data/*/*t1ce.nii.gz'))
flair_list = sorted(glob.glob('Data/*/*flair.nii.gz'))
mask_list = sorted(glob.glob('Data/*/*seg.nii.gz'))
...
for img in range(len(t2_list)):   #Using t1_list as all lists are of same size
    ...
    if (1 - (counts[0]/counts.sum())) > 0.01:  #At least 1% useful volume with labels that are not 0
        print("Save Me")
        temp_mask= to_categorical(temp_mask, num_classes=4).astype(np.float32)
        temp_combined_images = temp_combined_images.astype(np.float32)
